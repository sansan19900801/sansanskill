...
![version](https://img.shields.io/badge/version-0.3.0-222222)
...
```mermaid
flowchart LR
    A["交付当前信息<br/>问题 · 材料 · 目标"] --> B["sansan-business-router<br/>读取上下文并判断当前任务"]
    B --> C{"只选择当前最关键的一步"}
...
| 你现在的情况 | 系统会怎么处理 | 推荐入口 |
| --- | --- | --- |
| 有产品、有业务，但不知道先解决获客、成交还是交付 | 扫描八个商业系统，找出一个最大瓶颈、能力缺口和 7 天动作 | `sansan-business-router` |
...
| 不知道应该用哪个 Skill | 直接描述最近正在推进的一件事，由总入口选择当前一步 | `sansan-business-router` |
...
已经知道自己需要什么时，可以直接调用专项 Skill：

```text
使用 $sansan-business-diagnosis 帮我判断这个商业问题是否成立。
使用 $sansan-business-diagnosis 给我的业务做一次完整体检。
使用 $sansan-business-diagnosis 把这条内容收录成商业原子。
...
| Skill | 核心能力 | 典型结果 |
| --- | --- | --- |
| `sansan-business-router` | 读取完整上下文，识别当前最关键任务，路由到已经上线的专项 Skill | 一个明确入口和当前下一步 |
...
```bash
npx -y skills update sansan-business-router sansan-business-diagnosis sansan-wechat-moments-coach -g -y
```
...
`sansan-business-router` 不冒充万能专家。它只判断当前最值得推进的任务，并把完整上下文交给已经通过校验的专项 Skill。
