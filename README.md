...
仓库当前公开 **44 条结构化知识原子**和 **6 个 Skill 知识包**。所有公开原子都从可追溯的原始材料生成，再由构建流程整理为专项 Skill 可以调用的知识资产。

```mermaid
flowchart LR
    A["40 个原创朋友圈案例"] --> C["来源、权利与结构检查"]
    B["4 个商业观点与案例"] --> C
    C --> D["44 条结构化知识原子<br/>可检索 · 可引用 · 可继续扩充"]
    D --> E["6 个 Skill 知识包<br/>按真实业务场景重新组合"]
    E --> F["4 个可调用 Skills"]
    F --> G["/sansan 动态路由"]
```
...
- [原子库说明](知识库/原子库/原子库说明.md)：数据范围、字段、编号和维护规则；
- [朋友圈案例原子库](知识库/原子库/cases.jsonl)：40 个原创朋友圈案例的结构化数据；
- [商业原子库](知识库/原子库/business-atoms.jsonl)：可追溯的商业原则与真实业务案例；
- [Skill 知识包](知识库/Skill知识包/)：按朋友圈类型和商业诊断任务整理的 6 个知识包。

> 当前版本公开的是仓库知识库，不会自动把用户的对话、客户资料或商业诊断保存到本地。跨对话存档、恢复和报告能力将在对应 Skill 正式上线并通过验证后单独说明，不会提前写成已经具备的功能。
...
知识包不是另一份独立真源，而是由原子库按使用场景生成的可分发资产。新增或修改原子后，构建流程会重新生成对应知识包，避免原子库、Skill 和公开说明长期不一致。
